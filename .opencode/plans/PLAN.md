...
| Стъпка | Описание | Статус |
|--------|----------|--------|
| 1.B.1 | Създаване `lang/en/app.php` — 150+ ключа | ✅ |
| 1.B.2 | Създаване `lang/bg/app.php` — същите ключове на български | ✅ |
| 1.B.3 | `config/app.php` → `locale`, `fallback_locale`, `supported_locales` | ✅ |
...
| 1.C.2 | `change()` метод — валидация (`Rule::in(supported_locales)`), запис в session + user | ✅ |
| 1.C.3 | `POST /locale` route с име `locale.change` | ✅ |
| 1.C.4 | `SetLocale` middleware — чете locale: user → session → config, вика `App::setLocale()` | ✅ |
...
| Стъпка | Описание | Статус |
|--------|----------|--------|
| 1.D.1 | `HandleInertiaRequests` → share `locale`, `supportedLocales` | ✅ |
| 1.D.2 | `HandleInertiaRequests` → share `translations` (app.php + .json, merge-нати) | ✅ |
...
| Стъпка | Описание | Статус |
|--------|----------|--------|
| 1.E.1 | `useTranslation` hook — `__()`, `locale`, `supportedLocales` | ✅ |
| 1.E.2 | `LanguageSwitcher` — dropdown в sidebar footer | ✅ |
| 1.E.3 | LanguageSwitcher в SidebarFooter (между NavFooter и NavUser) | ✅ |
| 1.E.4 | Превод на dashboard страницата | ✅ |
| 1.E.5 | Превод на sidebar navigation текстовете | ✅ |
| 1.E.6 | Превод на всички settings страници (profile, security, appearance, teams) | ✅ |

##### 1.F — Filament Panels

| Стъпка | Описание | Статус |
|--------|----------|--------|
| 1.F.1 | Filament auto-използва `app()->getLocale()` за built-in UI | ✅ |
| 1.F.2 | `getModelLabel()` / `getPluralModelLabel()` — всички Resources (5 класа) | ✅ |
| 1.F.3 | `->label(__('...'))` на всички form fields, table columns, relation managers | ✅ |
| 1.F.4 | `TeamRole::label()` — `__()` за Admin/Member/Owner | ✅ |
